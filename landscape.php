...
try {
    ...
    if ($categoryId > 0) {
        ...
        // Lấy TẤT CẢ sản phẩm cảnh quan: cả category text VÀ category_id
        $whereClause = "WHERE ((p.category = 'Cảnh quan' OR p.category = 'cảnh quan') 
                        OR p.category_id IN (SELECT id FROM categories WHERE id = 3 OR parent_id = 3)) 
                        AND p.status = 1";
        $params = [];
    }
    ...
    $landscape = array_slice($allProducts, $offset, $limit);
    
    // Get landscape SUB-CATEGORIES (danh mục con) for filter
    $categoriesQuery = "
...
<body>
    ...
    <section class="materials-hero">
        <div class="hero-content">
            <h1 class="hero-title"><?php echo t('landscape_title'); ?></h1>
            <p class="hero-subtitle">
                <?php echo t('landscape_description'); ?>
            </p>
        </div>
    </section>
    ...
    <div class="main-content">
        <div class="category-filter">
            <div class="filter-header">
                ...
                <form method="GET" action="" class="search-box">
                    <input type="text" name="search" class="search-input" placeholder="<?php echo t('landscape_search_placeholder'); ?>" value="<?php echo htmlspecialchars($search); ?>">
                    <?php if ($categoryId > 0): ?><input type="hidden" name="category_id" value="<?php echo $categoryId; ?>"><?php endif; ?>
                    <button type="submit" class="search-btn" title="<?php echo t('btn_search'); ?>"><i class="fas fa-search"></i></button>
                </form>
            </div>
            <div class="category-pills">
                <a href="landscape.php<?php echo !empty($search) ? '?search=' . urlencode($search) : ''; ?>" 
                ...
                <?php foreach ($categories as $cat): 
                    ...
                    $url = 'landscape.php?category_id=' . $catId;
        ...
        <?php if (!empty($search) || $categoryId > 0): ?>
            <div style="margin-bottom: 2rem; text-align: center;">
                <h2 style="color: #1e293b; margin-bottom: 0.5rem;">
                    <?php echo t('landscape_found_results', ['count' => $totalItems]); ?>
                </h2>
                <?php if ($search): ?>
                    <p style="color: #64748b;"><?php echo t('search_keyword'); ?>: "<?php echo htmlspecialchars($search); ?>"</p>
                <?php endif; ?>
                <?php if ($categoryId > 0): ?>
                    <?php
                    $selectedCatName = '';
                    ...
                    ?>
                    <p style="color: #64748b;"><?php echo t('search_category'); ?>: <?php echo htmlspecialchars($selectedCatName); ?></p>
                <?php endif; ?>
            </div>
        <?php endif; ?>
                    ...
                    <div class="material-card" onclick="window.location.href='<?php echo buildProductUrl($item); ?>'">
                        ...
                        <div class="material-header">
                            ...
                            <?php else: ?>
                                <div class="material-image" style="background: linear-gradient(135deg, #38bdf8 0%, #22d3ee 100%); display: flex; align-items: center; justify-content: center;">
                                    <i class="fas fa-tree" style="color: white; font-size: 3rem; opacity: 0.7;"></i>
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="material-body">
                            ...
                            <p class="material-description">
                                <?php echo htmlspecialchars(substr($item['description'] ?? 'Sản phẩm cảnh quan chất lượng', 0, 80)); ?>
                        ...
                        <div class="material-footer">
                            <a href="<?php echo buildLangUrl('product-detail'); ?>?id=<?php echo $item['id']; ?>" class="view-material" onclick="event.stopPropagation();">
                                <span><?php echo t('btn_view_details'); ?></span>
        ...
        <?php else: ?>
            <div class="no-results">
                ...
                <h3><?php echo t('landscape_no_results'); ?></h3>
                <p><?php echo t('landscape_no_results_hint'); ?></p>
                <a href="landscape.php" style="color: #38bdf8; text-decoration: none; font-weight: 600; margin-top: 1rem; display: inline-block;">
                    ← <?php echo t('landscape_view_all'); ?>
                </a>
            </div>
        <?php endif; ?>
    </div>
